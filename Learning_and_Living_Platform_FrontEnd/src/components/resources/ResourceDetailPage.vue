...
>>>>>>> 1a10af13e08b8d0a8615deb0592c0a72382d68bc
          </div>
        </div>
        <v-dialog v-model="showDelete" :persistent="isDeleting" class="overlay">
          <div class="dialog-container">
    ...
    const id = computed(() => props.resId ?? Number(route.params.resId))
...
=======
    ...
    const tryDownload = async () => {
      ...
      try {
        // Call backend to get file meta (path + fileName)
        const meta = await new Promise((resolve, reject) => {
          post(
            '/resource/downloadResource',
            { resourceId: id.value },
            (message) => resolve(message),
            (msg, status) => reject(new Error(`${status ?? ''} ${msg ?? ''}`.trim()))
          )
        })

        const { path, fileName } = meta ?? {}
        if (!path) throw new Error('后端未返回文件地址')

        // If path is relative, go through Vite proxy (/api → backend)
        const isAbsolute = /^https?:\/\//i.test(path)
        const url = isAbsolute ? path : `/api${path.startsWith('/') ? '' : '/'}${path}`

        // Fetch blob and trigger download
        const resp = await axios.get(url, { responseType: 'blob' })
        saveBlob(resp.data, fileName || 'download')

        Snackbar.success('开始下载')
        await updateData()
      } catch (e) {
        console.error(e)
        Snackbar.error(`下载失败：${e?.message || e}`)
      } finally {
