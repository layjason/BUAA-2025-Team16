    ...
    <v-card class="users-info-box">
      <v-card-item title="用户信息">
        ...
        <v-btn
          ...
          style="height: 32px"
          color="#2F58CD"
          @click="router.push('/user/' + formData.id)"
        >
              ...
              <FormItem v-if="readonly">
                ...
                <template #label>
                  <Poptip trigger="hover" placement="right">
                    <template #title>
                      <p style="text-align: left; font-weight: bold">等级说明</p>
                    </template>
                    <template #content>
      ...
      <v-alert
        :type="hisAlert.type"
        variant="tonal"
        :title="hisAlert.title"
        :text="hisAlert.message"
        v-if="hisAlert.show"
        ...
        >
          <v-timeline-item v-for="(entry, index) in hisData" :key="index" size="small">
            <template v-slot:opposite>
              <v-btn
                variant="tonal"
                @click="moveToDetail(entry.resourceId)"
                title="打开相应资源详情页"
                >查看详情</v-btn
              >
            </template>
            <div style="display: flex; flex-direction: row; align-items: center">
              ...
              <div style="margin-left: 30px; width: 360px">
                <div class="small-title-dark" title="下载时资源标题">
                  {{ entry.title || '未命名资源' }}
                </div>
                <div class="text-dark" title="资源文件名">
                  {{ entry.fileName || '资源文件未命名' }}
                </div>
              </div>
            </div>
          </v-timeline-item>
...
const formData = reactive({
  ...
  levelName: '',
  profilePhotoUrl:
    'https://learning-and-living.oss-cn-beijing.aliyuncs.com/avatar/106115311_p0_master1200.jpg'
})
...
function updateAvatar(res) {
  ...
  put('user/updateUserInfo', formData, () => {
...
const passRules = {
  ...
  confirmPass: [{ validator: validatePassCheck, trigger: 'blur' }]
}
...
const updateHistory = async (hisPage) => {
  ...
  await get('/resource/listDownloadHistoryByUserId', (message) => {
    // console.log(message)
    ...
    if (message.length == 0) {
      showAlert(
        ...
        'info',
        '下载历史记录为空',
        '没有查询到对应的下载历史记录，可能您还未下载过教育资源，也可能您的下载记录已被清除，欢迎前往教育资源区下载资源。'
      )
      return
    }
